...
def visual():
    if request.method == "POST":
        print("POST request received at /visual")  # Debug line
        data = request.json
...
def predict_target_score():
    ...
    if venue_data.empty:
        return jsonify({"target_score_range": "170–185"})
    ...
    lower = max(130, int(base_target - (historical_std * 0.2)))  # 20% of std deviation
    upper = min(250, int(base_target + (historical_std * 0.2)))
